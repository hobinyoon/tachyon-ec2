...
set ylabel "Normalized time to Spark local"
...
INFILE u ($0 + _offset(0)):2:(BOXWIDTH) w boxes lt 1 lc rgb "red" t "spark - tachyon - local files. left - first, right - second and later", \
INFILE u ($0 + _offset(1)):3:(BOXWIDTH) w boxes lt 1 lc rgb "red" not, \
INFILE u ($0 + _offset(3)):4:(BOXWIDTH) w boxes lt 1 lc rgb "#006400" t "spark - tachyon - peer tachyon - local files. left - first, right - second and later", \
